(() => {
  ...
  // ── Modal de API key / proveedor ──────────────────
  const apikeyModal        = document.getElementById('apikey-modal');
  const apikeyProviders    = document.getElementById('apikey-providers');
  const apikeyFields       = document.getElementById('apikey-fields');
  const apikeyInput        = document.getElementById('apikey-input');
  const apikeyCustomFields = document.getElementById('apikey-custom-fields');
  const apikeyBaseUrlInput = document.getElementById('apikey-baseurl-input');
  const apikeyModelInput   = document.getElementById('apikey-model-input');
  ...
  let apikeySelectedProvider = null;

  function selectApikeyProvider(provider) {
    apikeySelectedProvider = provider;
    apikeyProviders.querySelectorAll('.sector-btn').forEach(b => {
      b.classList.toggle('selected', b.dataset.provider === provider);
    });
    apikeyFields.style.display = 'block';
    apikeyCustomFields.style.display = provider === 'custom' ? 'block' : 'none';
    const link = PROVIDER_KEY_LINKS[provider];
    document.getElementById('apikey-link-wrap').style.display = link ? 'block' : 'none';
    apikeyError.style.display = 'none';
    apikeyInput.focus();
  }

  apikeyProviders.querySelectorAll('.sector-btn').forEach(btn => {
    btn.addEventListener('click', () => selectApikeyProvider(btn.dataset.provider));
  });

  apikeyLink.addEventListener('click', e => {
    e.preventDefault();
    const link = PROVIDER_KEY_LINKS[apikeySelectedProvider];
    if (link) window.electronAPI?.openExternal(link);
  });

  function openApikeyModal(cancelable) {
    apikeySelectedProvider = null;
    apikeyProviders.querySelectorAll('.sector-btn').forEach(b => b.classList.remove('selected'));
    apikeyFields.style.display = 'none';
    apikeyInput.value = '';
    apikeyBaseUrlInput.value = '';
    apikeyModelInput.value = '';
    apikeyError.style.display = 'none';
    apikeyCancel.style.display = cancelable ? '' : 'none';
  ...
  // A diferencia de resumen-modal, no cerramos al clickear afuera cuando el
  // modal es obligatorio (sin botón cancelar) — si no, el onboarding se
  // podría "escapar" sin cargar una key.
  apikeyModal.addEventListener('click', e => {
    if (e.target === apikeyModal && apikeyCancel.style.display !== 'none') closeApikeyModal();
  });

  document.getElementById('apikey-settings-btn').addEventListener('click', () => openApikeyModal(true));

  apikeySave.addEventListener('click', async () => {
    const provider = apikeySelectedProvider;
    if (!provider) {
      apikeyError.textContent = 'Elegí un proveedor.';
      apikeyError.style.display = 'block';
      return;
    }
    const apiKey = apikeyInput.value.trim();
    if (!apiKey) {
      apikeyError.textContent = 'Ingresá una API key válida.';
      apikeyError.style.display = 'block';
      return;
    }
    const baseUrl   = apikeyBaseUrlInput.value.trim();
    const mainModel = apikeyModelInput.value.trim();
    if (provider === 'custom' && (!baseUrl || !mainModel)) {
      apikeyError.textContent = 'Para "Otro" completá la URL y el modelo.';
      apikeyError.style.display = 'block';
      return;
    }
    ...
    try {
      const res = await fetch('/settings/llm', {
        ...
        body: JSON.stringify({
          ...
          base_url: baseUrl, main_model: mainModel,
        }),
      });
      const data = await res.json();
      if (!data.ok) {
        apikeyError.textContent = data.error || 'No se pudo guardar la configuración.';
        apikeyError.style.display = 'block';
        return;
      }
      if (window.electronAPI?.saveLlmSettings) {
        await window.electronAPI.saveLlmSettings({
          provider, apiKey, baseUrl, mainModel,
        });
      }
      closeApikeyModal();
    } catch {
      apikeyError.textContent = 'No se pudo conectar con el servidor.';
      apikeyError.style.display = 'block';
    } finally {
...
${table.outerHTML}
  ...
  async function init() {
    ...
    // ── Onboarding: sin API key configurada, pedirla antes de nada ──
    try {
      const res = await fetch('/status');
      const { ready } = await res.json();
      if (!ready) openApikeyModal(false);   // sin cancelar, obligatorio
    } catch {
      // si /status falla no bloqueamos la UI — el chat mostrará su propio
      // error si el usuario intenta usarlo sin key configurada
    }
  }
